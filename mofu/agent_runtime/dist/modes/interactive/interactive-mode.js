...
function fitVisible(text, width) {
    ...
    for (const char of text) {
        if (visibleWidth(`${out}${char}…`) > width)
            break;
        out += char;
    }
    return padVisible(`${out}…`, width);
}
function readMofumofuReleaseStatus(cwd) {
    ...
    try {
        ...
        if (fs.existsSync(providerE2ePath)) {
            const providerE2e = JSON.parse(fs.readFileSync(providerE2ePath, "utf-8"));
...
class MofumofuStartupDashboard {
    cwd;
    constructor(cwd) {
        this.cwd = cwd;
    }
    ...
    render(width) {
        ...
        for (let i = 0; i < rows; i++) {
            ...
            lines.push(`${theme.fg("borderAccent", "│")} ${fitVisible(leftText, leftWidth)} ${theme.fg("borderMuted", "│")} ${fitVisible(rightText, rightWidth)} ${theme.fg("borderAccent", "│")}`);
        }
        lines.push(`${theme.fg("borderAccent", "╰")}${theme.fg("borderAccent", "─".repeat(leftWidth + 2))}${theme.fg("borderMuted", "┴")}${theme.fg("borderAccent", "─".repeat(rightWidth + 2))}${theme.fg("borderAccent", "╯")}`);
        return lines;
    }
}
