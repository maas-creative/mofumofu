...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|mofu] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Options:
  --self                  Update mofu only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall pi even if the current version is latest

Short forms:
  ${APP_NAME} update                Update mofu and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update mofu           Update mofu only (self works as an alias)
`);
			return;
